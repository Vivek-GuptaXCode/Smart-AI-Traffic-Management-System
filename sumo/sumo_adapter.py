        ...
        *,
        ...
        while steps < max_steps:
            try:
                ...
                # Call step callback with error isolation
                if on_step is not None:
                    try:
                        on_step(steps, sim_time, self._traci)
                    except Exception as callback_err:
                        # Log callback errors but don't crash the loop
                        if steps % 100 == 0:
                            print(f"[ADAPTER] Step callback error at step {steps}: {type(callback_err).__name__}")
                        # Don't break - continue with next step

                steps += 1
                connection_error_count = 0  # Reset error counter on success
                
            except Exception as step_err:
                connection_error_count += 1
                print(f"[ADAPTER] Step error (attempt {connection_error_count}/{max_connection_errors}): {type(step_err).__name__}: {str(step_err)[:80]}")
                
                if connection_error_count >= max_connection_errors:
                    print(f"[ADAPTER] Max connection errors reached. Stopping simulation.")
                    break
