...
class RLSignalController:
    ...
    def _resolve_model_run_dir(self) -> Path:
        if self._model_dir is None:
            raise FileNotFoundError("model_dir is not configured")

        direct_run_dir = self._model_dir
        if (direct_run_dir / "weights.npz").exists() and (direct_run_dir / "meta.json").exists():
            return direct_run_dir

        latest_run_dir = self._model_dir / "latest"
        if (latest_run_dir / "weights.npz").exists() and (latest_run_dir / "meta.json").exists():
            return latest_run_dir

        raise FileNotFoundError(f"No RL weights found under {self._model_dir}")

    def _load_saved_agent(self) -> DQNAgent | ImprovedDQNAgent:
        run_dir = self._resolve_model_run_dir()
        ...
        if agent_version.startswith(("improved_dqn", "enhanced_dqn")):
            return ImprovedDQNAgent.load(model_root, run_id=run_id)

        try:
            return DQNAgent.load(model_root, run_id=run_id)
        except Exception:
            return ImprovedDQNAgent.load(model_root, run_id=run_id)
