...
class SimpleActuatedPolicy:
    ...
    def select_action(
        ...
        *,
        ...
        current_phase_density = _safe_mean(halting[start_idx:end_idx])

        # Cross-phase density (all other lanes)
        mask = np.ones(self.max_lanes, dtype=bool)
        mask[start_idx:end_idx] = False
        cross_density = _safe_mean(halting[mask])
...
class MaxPressurePolicy:
    ...
    def _phase_queue_pair(
        ...
        current_queue = _safe_mean(halting[current_start:current_end])
        next_queue = _safe_mean(halting[next_start:next_end])
